...
def main() raises:
    ...
    _check(_render("Hello {{ name }}!", '{"name":"World"}') == "Hello World!", "var substitution")
    ...
    # ── elif ──────────────────────────────────────────────────────────────
    var grade = "{% if n == 1 %}one{% elif n == 2 %}two{% else %}many{% endif %}"
    _check(_render(grade, '{"n":2}') == "two", "elif branch")
    _check(_render(grade, '{"n":9}') == "many", "elif else branch")

    # ── for loop ──────────────────────────────────────────────────────────
    _check(
        _render("{% for i in items %}{{ i }}{% endfor %}", '{"items":["a","b","c"]}') == "abc",
        "for loop over list",
    )
    # for over a list of dicts with member access + a separator
    var msgs = "{% for m in messages %}{{ m['role'] }}:{{ m['content'] }};{% endfor %}"
    _check(
        _render(msgs, '{"messages":[{"role":"user","content":"hi"},{"role":"bot","content":"yo"}]}')
        == "user:hi;bot:yo;",
    ...
    # ── tojson filter ─────────────────────────────────────────────────────
    _check(_render("{{ o | tojson }}", '{"o":{"k":1}}') == '{"k": 1}', "tojson filter")
